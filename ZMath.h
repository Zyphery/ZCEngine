...
namespace ZCPP
{
	...
	Vector2D operator ! (Vector2D& const A) { return Vector2D(abs(A.x), abs(A.y)); }
	...
	Vector3D operator ! (Vector3D& A) { return Vector3D(abs(A.x), abs(A.y), abs(A.z)); }
	...
	Quaternion operator - (Quaternion& A) { return Quaternion(-A.w, -A.x, -A.y, -A.z); }
	...
	Quaternion RotationQuaternion(float r, Vector3D A) { Quaternion q; q.w = r; q.x = A.x; q.y = A.y; q.z = A.z; ~q; return q; }
	// Creates a rotation Quaternion using r (radians) and A (Vector3D Axis)

	Vector3D QuaternionRotate(float r, Vector3D V, Vector3D A) { Quaternion q = RotationQuaternion(r, A); Vector3D u = Vector3D(q.x, q.y, q.z); return 2 * DP3D(u, V) * u + (pow(q.w, 2) - DP3D(u, u)) * V + 2 * q.w * CP3D(u, V); }
	// Rotates V (Vector3D) around A (axis) by r (radians) using a Rotation Quaternion created with r & A

	Quaternion operator ! (Quaternion& A) { return Quaternion(abs(A.w), abs(A.x), abs(A.y), abs(A.z)); }
	// absolute value of Vector3D A
};
